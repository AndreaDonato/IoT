...
int main(int argc, char const *argv[])
{
    ...
    if(argc>=11){ P.gamma=atof(argv[9]); }
    if(argc>=12){ steps=atoi(argv[10]); }
    if(argc>=13){ seed=(unsigned int)strtoul(argv[11],NULL,10); }
    if(argc>=14){ simulations=atoi(argv[12]); }
    ...
    State s0 = (State){ .n1=0, .n2=0, .g1=1 };
    int *N=(int*)calloc(100, sizeof(int));                           // stato iniziale di simulazione (strade vuote, TL1 verde)
    ...
    for(int i=0; i<simulations; i++){
        ...
        int R = mdp_simulate(&P, s0, Pi, steps, &s, N);
    ...
    free(V); free(Pi); free(N);                                                     // libera la memoria dinamica
